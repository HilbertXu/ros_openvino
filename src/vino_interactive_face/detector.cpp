...
BaseDetection::BaseDetection(const std::string &topoName,
                             ...
                             bool doRawOutputMessages, bool enabled)
    : topoName(topoName), pathToModel(pathToModel), deviceForInference(deviceForInference),
      ...
      enablingChecked(false), _enabled(enabled), doRawOutputMessages(doRawOutputMessages) {
    if (isAsync) {
...
bool BaseDetection::enabled() const  {
    if (!enablingChecked) {
        _enabled = (!pathToModel.empty() && _enabled);
        if (!_enabled) {
...
FaceDetection::FaceDetection(const std::string &pathToModel,
    ...
    : BaseDetection("Face Detection", pathToModel, deviceForInference, maxBatch, isBatchDynamic, isAsync, doRawOutputMessages, isFaceEnabled),
